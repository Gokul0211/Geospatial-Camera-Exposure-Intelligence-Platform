...
### 2. Video AI Detection & Motion Geometry

#### Vector Line Perimeter Breach Detection
A line segment $L = (P_1, P_2)$ separates space into two half-planes. The signed orientation of point $Q = (x_q, y_q)$ relative to vector $\vec{P_1 P_2}$ is given by the 2D cross product determinant:

$$S(Q, P_1, P_2) = (x_2 - x_1)(y_q - y_1) - (y_2 - y_1)(x_q - x_1)$$

A breach is declared at step $k$ if and only if:

$$\text{Sign}\left(S(Q_k, P_1, P_2)\right) \neq \text{Sign}\left(S(Q_{k-1}, P_1, P_2)\right) \quad \text{and} \quad \text{Sign}(S) \neq 0$$

#### Polygon Ray-Casting Loitering Dwell Time
A object centroid $Q(x, y)$ is inside a polygon $V = \{v_1, v_2, \dots, v_n\}$ if the number of ray intersections going rightward $(x \to \infty)$ is odd:

$$I(Q, V) = \sum_{i=1}^n \mathbf{1}\left[ \left( (y_i > y) \neq (y_{i+1} > y) \right) \land \left( x < \frac{(x_{i+1} - x_i)(y - y_i)}{y_{i+1} - y_i} + x_i \right) \right]$$

Dwell time $D(i, Z) = t_{\text{current}} - t_{\text{entry}}(i, Z)$. Loitering alert fires when $D(i, Z) \ge T_{\text{dwell\_threshold}}$ (default $10\,\text{s}$).

---

### 3. Cryptographic Merkle Audit Ledger

Every detection alert $A_k$ is linked to an append-only SHA-256 Merkle hash chain:

$$H_0 = \text{SHA256}("COBRA\_WATCH\_GENESIS\_BLOCK")$$

$$H_k = \text{SHA256}\left( H_{k-1} \parallel \text{CanonicalJSON}(A_k) \right)$$

If any historic record $A_j$ ($j \le k$) is modified or tampered with, recalculating the chain yields $H'_k \neq H_k$, triggering immediate cryptographic tamper detection.
